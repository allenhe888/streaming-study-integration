...
private[spark] class KafkaRDD[K, V](
  ...
  private class KafkaRDDIterator(
    ...
    val kafkaPollCounter = new AtomicInteger(0)
    ...
    override def hasNext(): Boolean = if(requestOffset < part.untilOffset){
      true
    }else{
      val size = StaticCounter.getCache(this.consumer.consumer).size
      val sum = StaticCounter.getCache(this.consumer.consumer).sum()
      StaticCounter.cleanCache

      printf("分区-"+part.partition+" KafkaRDDIterator 指定数量消费完毕, kafkaConsumer.poll()次数:"+kafkaPollCounter.get()+", pollServer.times="+size +", pollServer.userTime="+sum)
      false
    }

    override def next(): ConsumerRecord[K, V] = {
      assert(hasNext(), "Can't call getNext() once untilOffset has been reached")
      val r = consumer.get(requestOffset, pollTimeout,kafkaPollCounter)
      requestOffset += 1
      r
    }

    def getPollCount():Int={
      kafkaPollCounter.get()
    }
