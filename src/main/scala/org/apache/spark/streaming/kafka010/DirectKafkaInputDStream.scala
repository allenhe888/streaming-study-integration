...
class DirectKafkaInputDStream[K, V](
  ...
  def resetOffsetRange(reFromOffset:Long)={
    val copyParams = new ju.HashMap[String, Object](executorKafkaParams)
    copyParams.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG,"earliest")
    if(copyParams.containsKey(ConsumerConfig.GROUP_ID_CONFIG)){
      copyParams.put(ConsumerConfig.GROUP_ID_CONFIG,"ResetOffset_"+copyParams.get(ConsumerConfig.GROUP_ID_CONFIG))
    }else{
      copyParams.put(ConsumerConfig.GROUP_ID_CONFIG,"ResetOffset_commGroupId")
    }
    val tmpKafkaConsumer = new KafkaConsumer[K, V](copyParams)
    this.synchronized {
      val topics = currentOffsets.keySet.map(_.topic()).toList.asJava
      tmpKafkaConsumer.subscribe(topics)
      tmpKafkaConsumer.poll(0)
      val topicPartitions = tmpKafkaConsumer.assignment()
      tmpKafkaConsumer.seekToBeginning(topicPartitions)
      currentOffsets =topicPartitions.asScala.map(tp=>{(tp,0)}).toMap.map(tp=>(tp._1,tmpKafkaConsumer.position(tp._1)))
    }
    tmpKafkaConsumer.close()
//    currentOffsets =currentOffsets.map(tp=>(tp._1,reFromOffset))
    System.err.println("Driver端重置了currentOffsets -> kc.beginningOffsets(): "+currentOffsets)
  }
  ...
  protected def latestOffsets(): Map[TopicPartition, Long] = {
    ...
    if(newPartitions.size>0){
      println("有kafak的Assignment变化了,1.Rebalance 前: ")
      println(newPartitions)
      newPartitions.foreach(key=>{
        println(key + " -> "+currentOffsets(key))
      })
    }
    // position for new partitions determined by auto.offset.reset if no commit
    currentOffsets = currentOffsets ++ newPartitions.map(tp => tp -> c.position(tp)).toMap
    if(newPartitions.size>0){
      println(" 2. Rebalance 后: ")
    ...
    c.seekToEnd(currentOffsets.keySet.asJava)// 发生Rebalance导致assginment减少时,currentOffsets并不会减少; 用未assigned的tp去seek就会抛""No current assignment for partition"异常;
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V]] = {
    ...
    val offsetRanges = untilOffsets.map { case (tp, uo) =>
      ...
      OffsetRange(tp.topic, tp.partition, fo, uo)//某TP在currentOffsets(tp) > untilOffsets(tp)
    }
